package company

import (
	"database/sql"
	"net/http"

	auditcontracts "mini-erp/internal/modules/audit/contracts"
	"mini-erp/internal/modules/company/application"
	"mini-erp/internal/modules/company/contracts"
	"mini-erp/internal/modules/company/infrastructure"
	"mini-erp/internal/modules/company/presentation"
)

// Module wires the company bounded context.
type Module struct {
	svc     *application.Service
	handler *presentation.Handler
}

// NewModule builds the company module over its own tables.
func NewModule(db *sql.DB, audit auditcontracts.AuditClient) *Module {
	svc := application.NewService(infrastructure.NewRepository(db), audit)
	return &Module{svc: svc, handler: presentation.NewHandler(svc)}
}

// Company exposes the public contract to other modules.
func (m *Module) Company() contracts.CompanyClient {
	return m.svc
}

// Service exposes use cases to the dev seed (composition root privilege).
func (m *Module) Service() *application.Service {
	return m.svc
}

// RegisterRoutes mounts endpoints with the auth-built permission wrapper.
func (m *Module) RegisterRoutes(mux *http.ServeMux, perm presentation.Permission) {
	presentation.RegisterRoutes(mux, m.handler, perm)
}
