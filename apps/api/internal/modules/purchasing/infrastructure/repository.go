...
// Repository owns the purchasing module tables.
...
const orderColumns = `id, number, branch_id, party_id, order_date, due_date,
	payment_terms, tax_type, tax_rate, subtotal, discount_total, tax_total,
	grand_total, status, notes, supplier_invoice_number, supplier_invoice_date`

func scanOrder(row interface {
	Scan(dest ...any) error
}, o *contracts.PurchaseOrder) error {
	var orderDate sql.NullTime
	var due, invDate sql.NullTime
	var notes, invNo sql.NullString
	var rate string
	err := row.Scan(&o.ID, &o.Number, &o.BranchID, &o.PartyID, &orderDate, &due,
		&o.PaymentTerms, &o.TaxType, &rate, &o.Subtotal, &o.DiscountTotal,
		&o.TaxTotal, &o.GrandTotal, &o.Status, &notes, &invNo, &invDate)
	if orderDate.Valid {
		o.OrderDate = orderDate.Time.Format("2006-01-02 15:04:05")
	}
	o.DueDate, o.Notes = formatNullTime(due), notes.String
	o.TaxRate = toFloat(rate)
	o.SupplierInvoiceNumber = invNo.String
	o.SupplierInvoiceDate = formatNullTime(invDate)
	return err
}
...
func (r *Repository) GetOrder(ctx context.Context, id int64) (*contracts.PurchaseOrder, error) {
	o := &contracts.PurchaseOrder{}
	err := scanOrder(r.db.QueryRowContext(ctx,
		"SELECT "+orderColumns+" FROM purchase_orders WHERE id = ?", id), o)
...
func (r *Repository) ItemsByOrder(ctx context.Context, orderID int64) ([]*contracts.Item, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, product_id, variant_id, product_code, product_name, uom,
		 ...
		 FROM purchase_order_items WHERE order_id = ? ORDER BY id ASC`, orderID)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) CreateOrder(ctx context.Context, o *contracts.PurchaseOrder, actorID int64) (int64, error) {
	...
	res, err := tx.ExecContext(ctx,
		`INSERT INTO purchase_orders
		 (number, branch_id, party_id, order_date, due_date, payment_terms,
		  tax_type, tax_rate, subtotal, discount_total, tax_total, grand_total,
		  status, notes, supplier_invoice_number, supplier_invoice_date,
		  created_by, updated_by)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?)`,
		o.Number, o.BranchID, o.PartyID, o.OrderDate, nullIfEmpty(o.DueDate),
		o.PaymentTerms, o.TaxType, o.TaxRate, o.Subtotal, o.DiscountTotal,
		o.TaxTotal, o.GrandTotal, nullIfEmpty(o.Notes),
		nullIfEmpty(o.SupplierInvoiceNumber), nullIfEmpty(o.SupplierInvoiceDate),
		actorID, actorID)
	if err != nil {
		return 0, err
	}
...
func (r *Repository) UpdateOrder(ctx context.Context, o *contracts.PurchaseOrder, actorID int64) error {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE purchase_orders SET party_id = ?, order_date = ?, due_date = ?,
		 payment_terms = ?, tax_type = ?, tax_rate = ?, subtotal = ?,
		 discount_total = ?, tax_total = ?, grand_total = ?, notes = ?,
		 supplier_invoice_number = ?, supplier_invoice_date = ?,
		 updated_at = UTC_TIMESTAMP(6), updated_by = ? WHERE id = ?`,
		o.PartyID, o.OrderDate, nullIfEmpty(o.DueDate), o.PaymentTerms,
		o.TaxType, o.TaxRate, o.Subtotal, o.DiscountTotal, o.TaxTotal,
		o.GrandTotal, nullIfEmpty(o.Notes),
		nullIfEmpty(o.SupplierInvoiceNumber), nullIfEmpty(o.SupplierInvoiceDate),
		actorID, o.ID); err != nil {
		return err
	}
...
func replaceItems(ctx context.Context, tx *sql.Tx, orderID int64, items []*contracts.Item) error {
	if _, err := tx.ExecContext(ctx, "DELETE FROM purchase_order_items WHERE order_id = ?", orderID); err != nil {
		return err
	}
...
func (r *Repository) SetStatus(ctx context.Context, id int64, status string, actorID int64) error {
	_, err := r.db.ExecContext(ctx,
		"UPDATE purchase_orders SET status = ?, updated_at = UTC_TIMESTAMP(6), updated_by = ? WHERE id = ?",
		status, actorID, id)
	return err
}

// List searches numbers with optional status filter, newest first.
func (r *Repository) List(ctx context.Context, branchID int64, search, status string, limit, offset int) ([]*contracts.PurchaseOrder, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		"SELECT "+orderColumns+" FROM purchase_orders WHERE "+strings.Join(conds, " AND ")+
			" ORDER BY id DESC LIMIT ? OFFSET ?", args...)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	var out []*contracts.PurchaseOrder
	for rows.Next() {
		o := &contracts.PurchaseOrder{}
...
func (r *Repository) Count(ctx context.Context, branchID int64, search, status string) (int64, error) {
	conds := []string{"branch_id = ?"}
	args := []any{branchID}
	if status != "" {
		conds = append(conds, "status = ?")
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM purchase_orders WHERE "+strings.Join(conds, " AND "), args...).Scan(&total)
	return total, err
}
...
func (r *Repository) ListByParty(ctx context.Context, branchID, partyID int64) ([]*contracts.OrderSummary, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, number, party_id, order_date, due_date, grand_total, status FROM purchase_orders
...
func (r *Repository) GetOrderByNumber(ctx context.Context, branchID int64, number string) (*contracts.PurchaseOrder, error) {
	o := &contracts.PurchaseOrder{}
	err := scanOrder(r.db.QueryRowContext(ctx,
		"SELECT "+orderColumns+" FROM purchase_orders WHERE branch_id = ? AND number = ?", branchID, number), o)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	return o, err
...
func (r *Repository) ListSummaries(ctx context.Context, branchID int64, statuses []string, from, to string, limit int) ([]*contracts.OrderSummary, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, number, party_id, order_date, due_date, grand_total, status
		 FROM purchase_orders WHERE `+strings.Join(conds, " AND ")+` ORDER BY id DESC LIMIT ?`, args...)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
